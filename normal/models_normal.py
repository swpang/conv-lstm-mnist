...
class CustomCNN(nn.Module):
    def __init__(self, rnn_input_dim, block, cnn_layers):
        ...
        self.layer3 = self.make_layer(block, 64, cnn_layers[2])
        self.avg_pool = nn.AvgPool2d(8)
        self.fc = nn.Linear(64, self.rnn_input_dim)
...
class LSTM(nn.Module):
    def __init__(self, input_dim, hidden_size, vocab_size, dropout, num_layers=1, bidirectional=True):
        ...
        self.lstm = nn.LSTM(input_size=self.hidden_size, hidden_size=self.hidden_size, 
                       num_layers=self.num_layers, batch_first=True, bidirectional=self.bidirectional, dropout=dropout)
...
class ConvLSTM(nn.Module):
    def __init__(self, rnn_input_dim, rnn_hidden_size, rnn_num_layers, 
        ...
        self.lstm = LSTM(self.rnn_input_dim, self.rnn_hidden_size, self.num_classes, 
                        self.dropout, self.rnn_num_layers, self.bidirectional)
